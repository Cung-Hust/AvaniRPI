...
def insertErrData(priority):
    ...
    try:
        with conn:
            cur.execute(f"INSERT INTO ERRORDATA VALUES({priority})")
    except:
...
def connect_Rabbit(message):
    connection = pika.BlockingConnection(
            pika.ConnectionParameters(
                RB_HOST, 5672, 
                '/', 
                pika.PlainCredentials(RB_USER, RB_PSWD),
                blocked_connection_timeout=1000,
                heartbeat=0,))
    channel = connection.channel()
    channel.queue_declare(queue='avani_cardreader')
    channel.basic_publish(
        exchange='', routing_key='avani.cardreader', body=message)
    print(message)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    tag = code.hex()
                    print("readerid:" + tag[16:30])
                    print("cardid:" + tag[38:42])
                    rfid_mess = {
                        "readerid": tag[16:30],
                        "time": int(time.time()),
                        "cardid": tag[38:42]
